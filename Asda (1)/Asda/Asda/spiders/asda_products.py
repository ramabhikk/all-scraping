    ...
    def parse_product(self, response, id):
        # try:
            ...
            for product in products:
                if not validate_value(product, ['price', 'upc']):
                    continue
                rollback = product['promotion_info'][0]['rollback']
                ...
                sale_price = item['Price'] if sale_price == '£0.0' else sale_price
                item['Sale Price'] = rollback['was_price'] if rollback else sale_price
                ...
                if rollback:
                    promotion = f'{promotion} | Rollback' if promotion else f'Rollback'
